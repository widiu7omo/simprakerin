            ...
            <table class="table table-bordered table-hover" id="example" width="100%" cellspacing="0">
              ...
              <tbody>
                ...
                <tr>
                  ...
                  <ul style="display: block; margin-left: 0; margin-right: 0; padding-left: 20px; width: 300px;" type="disk">
                    <?php
                    $query = $this->db->query("SELECT tb_perusahaan.lat_perusahaan,tb_perusahaan.long_perusahaan,tb_perusahaan.nama_perusahaan, tb_monev.status FROM tb_monev LEFT JOIN tb_perusahaan ON tb_monev.id_perusahaan = tb_perusahaan.id_perusahaan WHERE tb_monev.no_surat = '$key->no_surat'");
                    foreach ($query->result() as $data):
                    if ($data->nama_perusahaan != NULL) {
                      echo '<li><a href="https://www.google.com/maps/search/?api=1&query='.$data->lat_perusahaan.','.$data->long_perusahaan.'" title="Klik Untuk Melihat Google Map '.$data->nama_perusahaan.'" target="_blank">'.$data->nama_perusahaan.'</a><b> '.$data->status.'</b></li>';
                    }
                    endforeach; 
                    ?>
                  </ul>
                  ...
                  <td style="vertical-align: middle; text-align: center;">
                    <?php 
                      ...
                      echo tgl_indo($tgl_berangkat);
                    ?>
                  </td>
                  <td style="vertical-align: middle; text-align: center;">
                    <?php 
                      $date1  = date_create($key->tgl_pulang);
                      $tgl_pulang = date_format($date1,"Y-m-d");
                      echo tgl_indo($tgl_pulang); 
                    ?>  
                  </td>
                  <td style="vertical-align: middle; text-align: center;">
